...
public class MouseLookScript : MonoBehaviour {
	...
	private int cursorWidth = Screen.width/2, cursorHeight = Screen.height/2;
	private Vector2 cursorOffset;
	private GameObject inventory;
  private GameObject flashlight;

	//= new Vector2(cursorImage.w
	//idth / 2, cursorImage.height / 2);
	...
	void Awake(){
		...
		//cursorOffset = new Vector2(cursorImage.width / 2, cursorImage.height / 2);
		Debug.Log("Awake");
		myCamera = GameObject.FindGameObjectWithTag("MainCamera").transform;
		inventory = this.transform.Find("Inventory").gameObject;
    flashlight = this.transform.Find("FlashLight").gameObject;
		...
		if(Input.GetMouseButtonDown(0))
		{
			...
			if(Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit, 50, clickableLayer.value))
			{
				if(hit.collider.gameObject.tag == "Item")
				{
					...
					if(hit.collider.gameObject.name == "stickboy" || hit.collider.gameObject.name == "stickboy_temp")
					{
						Debug.Log("Picking up StickBoy");

						Destroy(hit.collider.gameObject);
						inventory.transform.Find("stickboy").gameObject.SetActive(true);


					}
					Debug.Log("Clickedat item");
				}


				else
				{
					Debug.Log("Nothing clikced");
				}
			}
...
void ApplyingStuff(){
  ...
  flashlight.transform.localRotation = Quaternion.Euler(currentCameraXRotation, 0, zRotation);
